...
- JSON schema v2：`{ "version": 2, "capsules": [ { id, text, cat: "work"|"life", tags[], createdAt, updatedAt, completedAt: ms|null, done: true|false, trashed: true|false } ] }`（兼容读取 v1：旧 `status` 的 `archived → done=true`、`trashed → trashed=true`；旧 v2 缺 `updatedAt` 时回填为 `createdAt`、`completedAt` 置 null）
